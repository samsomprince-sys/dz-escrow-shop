...
// 🌐 قاموس الترجمة الفوري للمنصة لضمان الاحترافية ومحاكاة G2A
const translations = {
  ar: {
  ...
  fr: {
    ...
    desc: "Le meilleur marché P2P et système de courtage financier (Escrow) для de vente de produits numériques via BaridiMob sécurisé.",
    ...
    loginLabel: "Se connecter / Inscription",
    brandName: "Wasata Escrow",
    langLabel: "Langue"
  },
  en: {
    title: "Algerian Escrow Platform 🇩🇿",
    desc: "The best P2P marketplace and secure financial escrow system for selling digital products via BaridiMob and crypto.",
...
export default function HomePage() {
  ...
  // لغة الموقع الافتراضية هي العربية
  const [lang, setLang] = useState<LangKey>('ar');

  const buyerFee = 500;
  const t = translations[lang]; // جلب نصوص اللغة المختارة فوراً
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 flex flex-col items-center" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* 🌐 شريط هيدر علوي مخصص لزر تسجيل الدخول وزر اللغات الذكي مثل G2A */}
      <div className="max-w-6xl w-full flex flex-wrap justify-between items-center mb-6 px-2 gap-4">
        ...
        <div className="flex items-center gap-3">
          {/* 🌐 زر وقائمة اختيار لغات المنصة الاحترافي المضافة حديثاً */}
          ...
          {/* 👤 زر حساب البائع بتصميم مطابقة لـ G2A */}
          <Link 
            href="/merchant/auth" 
      ...
      <div className={`max-w-6xl w-full ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
        <h2 className={`text-xl font-bold text-white mb-6 border-blue-600 pr-3 ${lang === 'ar' ? 'border-r-4' : 'border-l-4 pl-3'}`}>
          {t.marketTitle}
        </h2>
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between">
                    ...
                    <div className="mt-5 pt-4 border-t border-slate-800 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-lg font-bold text-white">{finalBuyerPrice} <span className="text-xs text-slate-400">{t.currency}</span></p>
                      </div>
                    </div>

                    <Link href={`/checkout/${product.id}`} className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl transition-all text-xs text-center">
                      {t.buyBtn}
                    </Link>
                  </div>
                </div>
